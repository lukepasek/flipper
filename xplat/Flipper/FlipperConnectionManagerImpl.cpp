...
namespace facebook
{
  namespace flipper
  {
    ...
    bool FlipperConnectionManagerImpl::connectSecurely()
    {
      ...
      bool useStaticCerts = !isCertificateDirectoryDefined();

      auto loadingDeviceId = flipperState_->start("Load Device Id");
      auto deviceId = useStaticCerts ? deviceData_.deviceId : contextStore_->getDeviceId();
      if (deviceId.compare("unknown") || deviceId.compare(deviceData_.deviceId))
      {
      ...
      if (useStaticCerts)
      {
        payload->csr = std::string("NOCSR").c_str();
        payload->csr_path = std::string("").c_str();
      }
      else
      {
    ...
    void FlipperConnectionManagerImpl::reconnect()
    {
      ...
      flipperScheduler_->scheduleAfter(
          [this]()
          { startSync(); },
          reconnectIntervalSeconds * 1000.0f);
    }
    ...
    {
      if (!isCertificateDirectoryDefined())
      {
        log("Connection context store root not defined, using stacicly configurad certificates - certificate exchange disabled!!!");
        return false;
      }
    ...
    bool FlipperConnectionManagerImpl::isCertificateDirectoryDefined()
    {
      return deviceData_.privateAppDirectory.length() > 0;
    }
