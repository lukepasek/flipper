...
namespace
{
  ...
  class JFlipperCertificateProvider
      : public jni::JavaClass<JFlipperCertificateProvider>
  {
  public:
    constexpr static auto kJavaDescriptor =
        "Lcom/facebook/flipper/core/FlipperCertificateProvider;";
  };

  class AndroidFlipperCertificateProvider : public FlipperCertificateProvider
  {
  public:
    AndroidFlipperCertificateProvider(
        jni::alias_ref<JFlipperCertificateProvider> certificateProvider);

  private:
    jni::global_ref<JFlipperCertificateProvider> jCertificateProvider;
  };
  ...
  public:
    ...
    static void registerNatives()
    {
      registerHybrid({
          ...
          // makeNativeMethod("setCertificateProvider", JFlipperClient::setCertificateProvider),
      });
    }
    ...
    void setCertificateProvider(
        jni::alias_ref<JFlipperCertificateProvider> provider)
    {
      __android_log_write(ANDROID_LOG_ERROR, "FLIPPER", "Mot yet implemented!!!");
      // try
      // {
      //   auto client = FlipperClient::instance();
      //   mCertificateProvider =
      //       std::make_shared<AndroidFlipperStateUpdateListener>(provider);
      //   client->setStateListener(mStateListener);
      // }
      // catch (const std::exception &e)
      // {
